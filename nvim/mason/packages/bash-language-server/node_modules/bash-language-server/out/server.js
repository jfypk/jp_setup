...
class BashServer {
    ...
    register(connection) {
        ...
        connection.onInitialized(() => __awaiter(this, void 0, void 0, function* () {
            ...
            if (environmentVariablesUsed.length > 0) {
                ...
                connection.console.warn(`Environment variable configuration is being deprecated, please use workspace configuration. The following environment variables were used: ${environmentVariablesUsed.join(', ')}`);
            }
    ...
    updateConfiguration(configObject) {
        if (typeof configObject === 'object') {
            try {
                ...
                if (!(0, util_1.isDeepStrictEqual)(this.config, newConfig)) {
                    ...
                    if (!shellcheckPath) {
                        this.connection.console.log('ShellCheck linting is disabled as "shellcheckPath" was not set');
    ...
    analyzeAndLintDocument(document) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (this.linter) {
                try {
                    const folders = ((_a = this.clientCapabilities.workspace) === null || _a === void 0 ? void 0 : _a.workspaceFolders)
                        ? yield this.connection.workspace.getWorkspaceFolders()
                        : [];
                    const { diagnostics: lintDiagnostics, codeActions } = yield this.linter.lint(document, folders || [], this.config.shellcheckArguments);
    ...
    onCodeAction(params) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const getDiagnosticsFingerPrint = (diagnostics) => (diagnostics &&
                diagnostics
                    .map(({ code, source, range }) => code !== undefined && source && range
                    ? JSON.stringify({
                        code,
                        source,
                        range,
...
function getCommandOptions(name, word) {
    ...
    const options = (0, child_process_1.spawnSync)(path.join(__dirname, '../src/get-options.sh'), [name, word]);
    if (options.status !== 0) {
        return [];
    }
